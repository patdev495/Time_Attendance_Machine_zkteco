...
public class ReflectionDumper
{
    ...
    {
        string path = @"C:\Program Files (x86)\Hanvon\FaceAtt\Client\HwDevOp.dll";
        Assembly asm = Assembly.LoadFile(path);
        Type type = asm.GetType("Hanvon.FaceID.DevTypeReflection");
        if (type == null)
        {
            Console.WriteLine("Type Hanvon.FaceID.DevTypeReflection not found");
            return;
        }

        string[] methodsToDump = { "InitDevTypeDict", "GetDictFromFile", "GetDictFromFile1", "GetHWDevType", "GetSortedDevTypeStrList" };
