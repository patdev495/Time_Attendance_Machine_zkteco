...
public class GetRecordDumper
{
    public static void Main()
    {
        ...
        if (type == null)
        {
            Console.WriteLine("Type DeviceOperation not found in KMS.Common.dll");
            return;
        }

        MethodInfo method = type.GetMethod("GetRecord", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static);
        if (method == null)
        {
            Console.WriteLine("Method GetRecord not found");
            return;
        }

        Console.WriteLine("=== Disassembling DeviceOperation.GetRecord ===");
        DumpMethod(method);
    }
