...
[System.AppDomain]::CurrentDomain.GetAssemblies() | ForEach-Object {
    ...
    foreach ($t in $types) {
        if ($t.FullName -eq "Hanvon.KMS.Business.BDeviceRelated") {
            Write-Host "Found BDeviceRelated!"
            Write-Host "  Assembly Full Name: $($t.Assembly.FullName)"
            Write-Host "  Assembly Location : $($t.Assembly.Location)"
            Write-Host "  Declaring Type    : $($t.DeclaringType)"
        }
    }
}
